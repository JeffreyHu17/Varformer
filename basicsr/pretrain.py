...
def init_loggers(opt):
    log_file = osp.join(opt['path']['log'], f"pretrain_{opt['name']}.log")
...
def create_pretrain_dataloader(opt, logger):
    pretrain_loader = None
    for phase, dataset_opt in opt['datasets'].items():
        dataset_enlarge_ratio = dataset_opt.get('dataset_enlarge_ratio', 1)
        train_set = build_dataset(dataset_opt)
        pretrain_sampler = EnlargedSampler(train_set, opt['world_size'], opt['rank'], dataset_enlarge_ratio)
    ...
    return pretrain_loader, pretrain_sampler, total_epochs, total_iters
...
def train_pipeline(root_path):
    ...
    result = create_pretrain_dataloader(opt, logger)
    pretrain_loader, pretrain_sampler, total_epochs, total_iters = result
    ...
    prefetch_mode = opt['datasets']['pretrain'].get('prefetch_mode')
    if prefetch_mode is None or prefetch_mode == 'cpu':
        prefetcher = CPUPrefetcher(pretrain_loader)
    elif prefetch_mode == 'cuda':
        prefetcher = CUDAPrefetcher(pretrain_loader, opt)
        logger.info(f'Use {prefetch_mode} prefetch dataloader')
        if opt['datasets']['pretrain'].get('pin_memory') is not True:
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        prefetcher.reset()
        pretrain_data = prefetcher.next()
        while pretrain_data is not None:
